...
class GTRANBRel(nn.Module):
    def __init__(self,
        ...
        self.layer_output = []
        # input projection (no residual)
        self.gat_layers.append(GTLayer(in_dim, num_hidden, pred_dim, heads[0],feat_drop, attn_drop, residual, batch_norm, edge_mode))
    ...
    def forward(self, blocks, inputs):
        ...
        if self.scale:
            centrality = blocks[-1].dstdata['centrality']  # centrality adjustment
            logits = nn.functional.relu((centrality * self.gamma + self.beta).unsqueeze(-1) * logits)   # activation function
    ...
    def inference(self, g, x, batch_size, num_workers, device):
        ...
        for l, layer in enumerate(self.gat_layers):
            ...
            sampler = dgl.dataloading.MultiLayerFullNeighborSampler(1)    #  sample process
