...
class CrossModalAttentionFusion(nn.Module):
    def __init__(self, struct_dim, semantic_dim, num_heads=4, dropout=0.1):
        super(CrossModalAttentionFusion).__init__()
...
class Gatefusion(nn.Module):
    def __init__(self, struct_dim, semantic_dim):
        ...
        self.linear_struct = nn.Linear(struct_dim, struct_dim)
        self.linear_semantic = nn.Linear(semantic_dim, struct_dim)
        self.sigmoid = nn.Sigmoid()
    
    def forward(self, struct_feats, semantic_feats):
        # 计算门控值
        gate_values = self.sigmoid(self.linear_struct(struct_feats) + self.linear_semantic(semantic_feats))
        # 融合特征
        fused_feats = gate_values * struct_feats + (1 - gate_values) * semantic_feats
        return fused_feats
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, struct_feats, semantic_feats):
        # 计算注意力权重
        scores = torch.cat([self.linear_struct(struct_feats), self.linear_semantic(semantic_feats)], dim=1)
        attn_weights = self.softmax(scores)
        # 融合特征
        fused_feats = attn_weights[:, 0:1] * struct_feats + attn_weights[:, 1:2] * semantic_feats
        return fused_feats
...
class ConcatFusion(nn.Module):
    ...
    def forward(self, struct_feats, semantic_feats):
        ...
        # 线性变换
        fused_feats = self.relu(self.linear(concat_feats))
        return fused_feats
    

class FixedWeightFusion(nn.Module):
    def __init__(self, struct_dim, semantic_dim, output_dim,  eta):
        super().__init__()
        self.alpha = eta
        self.relu = nn.ReLU()
    ...
    def forward(self, struct_feats, semantic_feats):
        ...
        fused_feats = self.alpha * struct_feats + (1 - self.alpha) * semantic_feats
        return fused_feats
...
class AdaptiveWeightedFusion(nn.Module):
    def __init__(self, struct_dim, semantic_dim, output_dim, eta):
        super().__init__()
        self.alpha = nn.Parameter(torch.tensor(eta))  # 可训练参数
...
class structure_semantics_fusion(nn.Module):
    def __init__(self, struct_dim, semantic_dim, output_dim, fusion_mode='concat', eta =0.2):
        ...
        if fusion_mode == 'concat':
            ...
            self.fusion_layer = FixedWeightFusion(struct_dim, semantic_dim, output_dim, eta)
        elif fusion_mode == 'adaptive':
            self.fusion_layer = AdaptiveWeightedFusion(struct_dim, semantic_dim, output_dim, eta)
